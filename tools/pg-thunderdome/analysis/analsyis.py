...
def _(my_scan_csv, pl):
    run3 = pl.concat([
        my_scan_csv("results/20260630/bolt-result-202606301026.csv", "bolt"),
        my_scan_csv("results/20260630/bolt-result-202606301114.csv", "bolt"),
        my_scan_csv("results/20260630/postgres17_result-202606301210.csv", "pg17"),
        my_scan_csv("results/20260630/postgres17_result-202606301216.csv", "pg17"),
        my_scan_csv("results/20260630/postgres18_result-202606301229.csv", "pg18"),
        my_scan_csv("results/20260630/postgres18_result-202606301235.csv", "pg18"),
        my_scan_csv("results/20260630/oriole17_result-202606301248.csv", "or17"),
        my_scan_csv("results/20260630/oriole17_result-202606301302.csv", "or17"),
        my_scan_csv("results/20260630/oriole18_result-202606301441.csv", "or18"),
        my_scan_csv("results/20260630/oriole18_result-202606301454.csv", "or18")
    ])
...
@app.cell
def _(alt, pl, run3_read_ops):
    _chart = (
        alt.Chart(run3_read_ops.filter((pl.col("database") == "bolt") | (pl.col("kv_type").str.contains("nifs"))).collect())
        .mark_bar()
        .encode(
            x=alt.X(field='value_size', type='nominal', title='Value Size (bytes)'),
            y=alt.Y(field='read_ops_per_s_avg', type='quantitative', title='Read Ops/S (Higher Is Better)'),
...
@app.cell
def _(pl, run3_read_ops):
    run3_read_response_times = run3_read_ops.filter((pl.col("database") == "bolt") | (pl.col("kv_type").str.contains("nifs"))).select(
        pl.col("config"),
        pl.col("value_size"),
        pl.col("read_slowest_response_s_avg"),
        pl.col("read_fastest_response_s_avg"),
    ...
    run3_read_response_times
    return (run3_read_response_times,)


@app.cell
def _(alt, run3_read_response_times):
    _chart = (
        alt.Chart(run3_read_response_times)
        .mark_bar(size=18)
        .encode(
            x=alt.X(field='value_size', type='nominal', title='Value Size (bytes)'),
            y=alt.Y(field='read_average_response_s_avg', type='quantitative', title='Read Latency (S) (Lower Is Better)'),
...
@app.cell
def _(alt, run3_write_ops):
    _chart = (
        alt.Chart(run3_write_ops.collect())
        .mark_bar()
        .encode(
            x=alt.X(field='value_size', type='nominal', title='Value Size (bytes)'),
            y=alt.Y(field='write_ops_per_s_avg', type='quantitative', title='Write Ops/S (Higher Is Better)', stack=False),
...
@app.cell
def _(alt, pl, run3_write_ops):
    run3_write_response_times = run3_write_ops.select(
        pl.col("config"),
        pl.col("value_size"),
        pl.col("write_slowest_response_s_avg"),
        pl.col("write_fastest_response_s_avg"),
    ...
    _chart = (
        alt.Chart(run3_write_response_times)
        .mark_bar()
        .encode(
            x=alt.X(field='value_size', type='nominal', title='Value Size (bytes)'),
            y=alt.Y(field='write_average_response_s_avg', type='quantitative', title='Write Latency (S) (Lower Is Better)'),
